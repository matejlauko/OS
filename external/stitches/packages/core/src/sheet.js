/** @typedef {import('../types/sheet').RuleGroup} RuleGroup */
/** @typedef {import('../types/sheet').RuleGroupNames} RuleGroupNames */
/** @typedef {import('../types/sheet').SheetGroup} SheetGroup */
...
export const createSheet = (/** @type {DocumentOrShadowRoot} */ root) => {
    ...
    return [].map
      .call(cssRules, (cssRule, cssRuleIndex) => {
        ...
        if (
          ...
          (lastRuleCssText = cssRules[cssRuleIndex - 1].cssText).startsWith('--sxs')
        ) {
  ...
  const reset = () => {
    ...
    if (!groupSheet) {
      ...
      groupSheet = {
        sheet: root
          ? (root.head || root).appendChild(document.createElement('style')).sheet
...
export const createRulesInjectionDeferrer = (globalSheet) => {
  ...
  names.forEach(
    (sheetName) =>
      (injector.rules[sheetName] = { apply: (rule) => injector[$pr].push([sheetName, rule]) })
  )
  return injector
}
