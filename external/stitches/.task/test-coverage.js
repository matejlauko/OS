...
!(async () => {
  ...
  for (const filename of await fs.readdir(NODE_V8_COVERAGE)) {
    results.push(
      ...JSON.parse(await fs.readFile(fp.join(NODE_V8_COVERAGE, filename), 'utf8')).result
    )
  }
  ...
  for (let result of results) {
    ...
    if (filter(url)) {
      ...
      lcovInfo += `${Object.entries(lcov).reduce(
        (report, [name, data]) =>
          report.concat(
            data.reduce((string, each) => string.concat(`${name.toUpperCase()}:${each}\n`), '')
          ),
        ''
      )}end_of_record\n`
    }
